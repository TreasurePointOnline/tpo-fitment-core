...
def force_delete_index_html():
    ...
    try:
        ...
        with open("temp_cache_buster.txt", "w") as f:
            f.write("cache bust " + str(time.time()))
        
        with open("temp_cache_buster.txt", "rb") as f:
            ftps.storbinary(f"STOR {temp_file_path}", f)
        ftps.delete(temp_file_path) # Delete it immediately
        os.remove("temp_cache_buster.txt")
