...
# PRISTINE WP-CONFIG CONTENT
# (No extra lines, no debug code, just pure config)
...
def restore_and_clean():
    ...
    try:
        ...
        # 1. Restore Index
        print("Restoring index.php...")
        try:
            ftps.delete("public_html/index.php") # Delete fake one
            ftps.rename("public_html/index.php.real", "public_html/index.php") # Bring back real one
            print("✅ index.php restored.")
        except Exception as e:
            print(f"⚠️ Index restore issue: {e}")

        # 2. Upload Clean Config
        print("Uploading CLEAN wp-config.php...")
        with open("wp-config_clean.php", "w") as f:
            f.write(WP_CONFIG_CONTENT)
            
        with open("wp-config_clean.php", "rb") as f:
            ftps.storbinary("STOR public_html/wp-config.php", f)
            
        print("✅ wp-config.php sanitized.")
        print("👉 Go check http://treasurepointonline.com")
